...
function FileManager({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  const concatenateVideos = async (entries: Entry[]) => {
    ...
    try { await launchInstalledApp(`${videoApp.launchUrl}?handoff=1`, (url, target) => window.open(url, target), () => api.launchApp(videoApp.id, 'concatenate', entries.map(entry => entry.id))); setError('') }
