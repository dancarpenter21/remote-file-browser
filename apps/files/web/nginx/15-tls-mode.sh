...
common_locations='root /usr/share/nginx/html;
    ...
    location ^~ /api/docs/ { proxy_pass http://files-server:8080; proxy_http_version 1.1; proxy_set_header Host $host; add_header X-Content-Type-Options nosniff always; add_header Referrer-Policy no-referrer always; add_header Content-Security-Policy "default-src '\''self'\''; img-src '\''self'\'' data:; style-src '\''self'\'' '\''unsafe-inline'\''; script-src '\''self'\'' '\''unsafe-inline'\''; connect-src '\''self'\''" always; }
    location /api/ { proxy_pass http://files-server:8080; proxy_http_version 1.1; proxy_set_header Host $host; proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for; proxy_set_header X-Forwarded-Proto $scheme; proxy_set_header Upgrade $http_upgrade; proxy_set_header Connection $connection_upgrade; proxy_read_timeout 3600s; proxy_request_buffering off; }
    location / { try_files $uri $uri/ /index.html; }'

if [ "${FILES_APPS_ENABLED:-true}" = "true" ]; then
  common_locations="${common_locations}
    location = /_rfb_app_auth { internal; proxy_pass http://files-server:8080/api/v1/auth/check; proxy_pass_request_body off; proxy_set_header Content-Length \"\"; proxy_set_header Cookie \$http_cookie; }"
fi
