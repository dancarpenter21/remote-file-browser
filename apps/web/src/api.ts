...
export async function getProject(id: string): Promise<Project> {
  return responseJson(await fetch(`/api/projects/${id}`));
}
...
export async function uploadCrowd(id: string, file: File): Promise<Project> {
  const form = new FormData();
  form.append("file", file);
  return responseJson(await fetch(`/api/projects/${id}/crowd`, { method: "POST", body: form }));
}

export async function createRender(id: string, kind: RenderKind, expectedRevision: number): Promise<RenderJob> {
  return responseJson(await fetch(`/api/projects/${id}/renders`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function cancelRender(id: string): Promise<RenderJob> {
  return responseJson(await fetch(`/api/jobs/${id}`, { method: "DELETE" }));
}
