...
export function App() {
  ...
  if (launch.get("handoff") === "1" && !activeId) {
    return <main className="library-page"><section className="library-hero"><span className="eyebrow">Remote Files</span><h1>Importing video…</h1><p>The source is being copied into the VFX workspace.</p></section></main>;
  }

  if (activeId && (loading || !active || active.status === "importing")) {
    return <main className="library-page"><section className="library-hero"><span className="eyebrow">VFX Editor</span><h1>Preparing project…</h1><p>{active?.name ?? "Loading the selected video."}</p></section></main>;
  }

  if (active?.status === "ready") {
    return <Editor initialProject={active} onBack={() => { history.replaceState(null, "", appUrl()); setActiveId(undefined); }} onProjectChange={(updated) => setProjects((all) => all.map((project) => project.id === updated.id ? updated : project))} />;
  }
